  ...
  return (
    <div className="space-y-8 py-4 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="p-3 bg-dark-card border border-dark-border rounded-2xl flex items-center gap-2 text-xs text-gray-300">
      ...
      {/* History List */}
      {isLoadingElections ? (
        <div className="py-16 text-center text-gray-400">Loading historical archives from Bohr Testnet...</div>
      ) : filteredHistory.length === 0 ? (
        <div className="py-16 text-center text-gray-400 bg-dark-card border border-dark-border rounded-3xl p-8 space-y-2">
          <Trophy className="w-12 h-12 text-gray-600 mx-auto" />
          <p className="text-base font-semibold text-gray-300">No Finalized Elections Found</p>
          ...
          {filteredHistory.map((election) => {
            ...
            return (
              <div
                ...
                className="p-6 sm:p-8 bg-dark-card border border-dark-border hover:border-emerald-500/40 rounded-3xl space-y-6 shadow-xl transition-all relative overflow-hidden"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono font-bold text-gray-400 bg-dark-surface px-2.5 py-0.5 rounded-full border border-dark-border">
                  ...
                  <button
                    ...
                    className="shrink-0 flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl transition-all cursor-pointer shadow-glow-subtle active:scale-95"
                  >
                ...
                {/* Winner Callout Card */}
                <div className="p-4 sm:p-5 bg-gradient-to-r from-amber-950/40 to-dark-surface rounded-2xl border border-amber-500/30 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                  <div>
                    <span className="text-amber-400 block text-[11px] font-semibold uppercase">Winner Status</span>
                  ...
                  <div>
                    <span className="text-amber-400 block text-[11px] font-semibold uppercase">Winning Vote Weight</span>
                  ...
                  <div>
                    <span className="text-amber-400 block text-[11px] font-semibold uppercase">Total Voter Turnout</span>
                ...
                {/* Verification Footer */}
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-gray-400 pt-2 border-t border-dark-border/60">
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-emerald-400" />
